...
void Draw_SignalDistribution(int xxx=0){
  ...
  TString region = "OneLepton_AwayFatJet";
  if(xxx==0){}
  else if(xxx==1){
    region = "TwoLepton_TwoJet";
  }
  else{
    return;
  }
  ...
  TString base_plotpath = ENV_PLOT_PATH+"/"+dataset+"/SignalDistribution/"+region+"/";
  ...
  for(unsigned int it_Config=0; it_Config<Configs.size(); it_Config++){
    ...
    TString this_plotpath = base_plotpath+"/"+Config+"/";
    ...
    for(unsigned int it_channel=0; it_channel<channels.size(); it_channel++){
      ...
      for(unsigned int it_var=0; it_var<vars.size(); it_var++){
        ...
        TString histname = Config+"_"+Suffix+"_"+region+"/"+var+"_"+Config+"_"+Suffix+"_"+region;
