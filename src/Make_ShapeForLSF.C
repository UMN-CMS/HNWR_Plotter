...
void Make_ShapeForLSF(int Year=2016){
...
*/
  ...
  TString ShapeVarName = "HNFatJet_LSF";
  int n_rebin = 1;
  ...
  TString base_filepath = WORKING_DIR+"/rootfiles/"+dataset+"/NoLSFCut/"+TString::Itoa(Year,10)+"/";
  TString base_plotpath = ENV_PLOT_PATH+"/"+dataset+"/ShapeForLSF/"+TString::Itoa(Year,10)+"/";
  ...
  map_sample_string_to_list["DY"] = {"DYJets_MG_HT_Reweighted"};
    ...
    for(unsigned int it_channel=0; it_channel<channels.size(); it_channel++){
      ...
      for(unsigned it_syst=0; it_syst<systs.size(); it_syst++){
        ...
        for(unsigned int it_sample=0; it_sample<samplelist.size(); it_sample++){
          ...
          //if(sample=="DYJets_MG_HT_Reweighted") sample = "DYJets_MG_HT_Reweighted_Reshaped";
