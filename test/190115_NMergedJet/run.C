...
"WRtoNLtoLLJJ_WR4000_N1000_13TeV_TuneCUETP8M1",
  ...
  for(unsigned int it_ch=0; it_ch<channels.size(); it_ch++){
    ...
    for(unsigned int it_sample=0; it_sample<samples.size(); it_sample++){
      ...
      TH1D *hist_NoCut_Full = (TH1D *)file_Full->Get(channel+"/GenStudy__fatjet_matched_gen_N__NotEmpty_"+channel);
      TH1D *hist_NLSFfatjet_Full = (TH1D *)file_Full->Get(channel+"/FatJet_LSF_Size_"+channel);
      //TH1D *hist_SNU = (TH1D *)file_Full->Get(channel+"/GenStudy__fatjet_matched_gen_N__PassSNUTag_"+channel);
      //TH1D *hist_UMN = (TH1D *)file_Full->Get(channel+"/GenStudy__fatjet_matched_gen_N__PassUMNTag_"+channel);

      hist_NLSFfatjet_Full->Scale(1./hist_NLSFfatjet_Full->GetEntries());
      cout << samplealias;
      for(int j=1; j<=4; j++){
        cout << "\t" << hist_NLSFfatjet_Full->GetBinContent(j);
      }
      cout << endl;

      //cout << samplealias << "\t" << hist_SNU->GetEntries()/hist_NoCut_Full->GetEntries() << "\t" << hist_UMN->GetEntries()/hist_NoCut_Full->GetEntries() << endl;
